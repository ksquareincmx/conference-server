...
import {
  IDialogParams,
  IDialogForAppointmentParams,
  IAppointmentInfo,
  IDateRoomConfirmationInfo
} from "../interfaces/v2/SlackInterfaces";
import { IRoomSlackFormat } from "../interfaces/v2/RoomInterfaces";
import { roomMapper } from "../mappers/v2/RoomMapper";

class Builder {
  confirmationMessage: () => {};

  dialog = async ({ type, defaultValues }: IDialogParams) => {
    ...
    return this.dialogForAppointment({
      roomsFormated,
      defaultValues
    });
  };

  dialogForDateAndRoom = (roomsFormated: IRoomSlackFormat[]) => {
    const today = moment()
      ...
      blocks: [
        ...
        },
        {
          ...
          text: {
            type: "plain_text",
            emoji: true,
            text: "This are the availabale hours: "
          }
        },
        {
          type: "section",
  ...
  buildAppointmentConfirmation = (bookingInfo: IAppointmentInfo) => {
    const {
      slackUserName,
      startDate,
      endDate,
