...
class AppointmentList extends React.Component {
  ...
  ClickedNextButtonHandler = () => {
    ...
    const post = {
      ...
      'start': this.state.date + 'T' + this.state.startTime.hour + ':' + this.state.startTime.minute + ':' + '00.000Z',
      'end': this.state.date + 'T' + this.state.endTime.hour + ':' + this.state.endTime.minute + ':' + '00.000Z',
      'attendees': [...this.state.attendees]
    }


    if (this.state.bookingClicked) {
      this.props.booking.modifyBooking(post, this.props.bookingClickedObj.bookingId).then(res => {
  ...
  componentDidMount() {
    ...
    if (this.props.quickAppointment) {
      ...
      if (!this.state.bookingClicked) {
        ...
        const startDateFormat = this.props.bookingClickedObj.startDate.slice(0, this.props.bookingClickedObj.startDate.length - 1)
        const startDate = new Date(startDateFormat)

        const endDateFormat = this.props.bookingClickedObj.endDate.slice(0, this.props.bookingClickedObj.endDate.length - 1)
        const endDate = new Date(endDateFormat)


        let date = (startDate.getFullYear() + '-' + this.AddZeros(startDate.getMonth() + 1) + '-' + this.AddZeros(startDate.getDate()))
